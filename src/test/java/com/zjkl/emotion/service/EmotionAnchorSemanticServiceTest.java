...
class EmotionAnchorSemanticServiceTest {
    ...
    void generateSemanticFields_shouldRequestOnlyLatestTwentyMessagesInRange() {
        EmotionAnchorSemanticService service = new EmotionAnchorSemanticService(
                qwenChatModel,
        ...
        EmotionAnchorEvent event = new EmotionAnchorEvent();
        event.setUserId("u1");
        ...
        verify(converMessageService).getLatestByTimeRange(eq("u1"), eq(event.getStartTime()), any(LocalDateTime.class), eq(EmotionAnchorSemanticService.CONTEXT_MESSAGE_LIMIT));
