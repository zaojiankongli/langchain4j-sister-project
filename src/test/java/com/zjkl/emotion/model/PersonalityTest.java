...
class PersonalityTest {
    ...
    void gentleAndShyBasePAD() {
        ...
        // P = 0.59*0 + 0.19*0 + 0.21*(-0.5) + 0.15*0.6 + (-0.57)*(-0.2) = 0.099
        double expectedP = 0.21 * -0.5 + 0.15 * 0.6 + (-0.57) * -0.2;
        assertEquals(expectedP, pad.getPleasure(), 1e-10);

        // Arousal范围[0,1]，计算结果-0.134被clamp到0
        assertEquals(0.0, pad.getArousal(), 1e-10);

        // D = 0.40*(-0.5) + 0.20*0.6 + (-0.51)*(-0.2) = 0.022
    ...
    void extremePersonalityStaysInRange() {
        ...
        assertTrue(pad.getArousal() >= 0.0 && pad.getArousal() <= 1.0);   // Arousal[0,1]
