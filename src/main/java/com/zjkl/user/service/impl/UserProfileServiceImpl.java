...
public class UserProfileServiceImpl implements UserProfileService {
    ...
    public void updateBasic(String userId, String username, Integer gender) {
        ...
        if (rows == 0) {
            throw new BusinessException(ErrorCode.INTERNAL_ERROR, "更新基本信息失败");
        }
    }
    ...
    public void updateHobbies(String userId, String hobbies) {
        ...
        if (rows == 0) {
            throw new BusinessException(ErrorCode.INTERNAL_ERROR, "更新爱好失败");
        }
    }
    ...
    public void updateAiType(String userId, Integer aiType) {
        ...
        if (rows == 0) {
            throw new BusinessException(ErrorCode.INTERNAL_ERROR, "更新AI类型失败");
        }
    }
    ...
    public String uploadAvatar(String userId, MultipartFile file) {
        ...
        try {
            // 调用 OSS 服务上传头像（无事务，不持有 DB 连接）
            String ossUrl = ossService.uploadAvatar(userId, file);
            
            // 上传成功后单独更新数据库（短事务）
            updateAvatarInDb(userId, ossUrl);
